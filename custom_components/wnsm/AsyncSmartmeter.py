...
from .const import (
    ATTRS_METERREADINGS_CALL, ATTRS_BASEINFORMATION_CALL, ATTRS_CONSUMPTIONS_CALL,
    ATTRS_BEWEGUNGSDATEN, ATTRS_ZAEHLPUNKTE_CALL, ATTRS_HISTORIC_DATA, ATTRS_VERBRAUCH_CALL
)
...
class AsyncSmartmeter:
    ...
    async def get_meter_readings(self) -> dict[str, any]:
        response = await self.hass.async_add_executor_job(self.smartmeter.historical_data)
        ...
        if contracts is not None and isinstance(contracts, list) and len(contracts) > 0:
            for contract in contracts:
                if "zaehlpunkte" in contract:
                    geschaeftspartner = contract.get("geschaeftspartner")
                    zaehlpunkte += [
    ...
    async def get_historic_data(self, zaehlpunkt: str, date_from: datetime = None, date_to: datetime = None, granularity: ValueType = ValueType.QUARTER_HOUR):
        response = await self.hass.async_add_executor_job(
            self.smartmeter.historical_data, zaehlpunkt, date_from, date_to, granularity
        )
        if "Exception" in response:
    ...
    async def get_meter_reading_from_historic_data(self, zaehlpunkt: str, start_date: datetime, end_date: datetime) -> float | None:
        """Return the latest meter reading (absolute cumulative kWh) from the API."""
        response = await self.hass.async_add_executor_job(
        ...
        if (
            "values" in meter_readings
            and len(meter_readings['values']) > 0
            and all("messwert" in messwert for messwert in meter_readings['values'])
        ):
    ...
    async def get_bewegungsdaten(self, zaehlpunkt: str, start: datetime = None, end: datetime = None, granularity: ValueType = ValueType.QUARTER_HOUR):
        response = await self.hass.async_add_executor_job(
            self.smartmeter.bewegungsdaten, zaehlpunkt, start, end, granularity
        )
        if "Exception" in response:
