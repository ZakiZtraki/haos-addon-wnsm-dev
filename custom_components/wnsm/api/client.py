...
class Smartmeter:
    ...
    def find_valid_obis_data(self, zaehlwerke: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for zaehlwerk in valid_data:
            if not zaehlwerk.get("messwerte"):
                ...
                logger.debug(f"Valid OBIS code '{obis}' has empty or missing messwerte.")
    ...
    def bewegungsdaten(
        ...
        if anlagetype == const.AnlagenType.FEEDING:
            rolle = const.RoleType.DAILY_FEEDING.value if valuetype == const.ValueType.DAY else const.RoleType.QUARTER_HOURLY_FEEDING.value
        else:
            rolle = const.RoleType.DAILY_CONSUMING.value if valuetype == const.ValueType.DAY else const.RoleType.QUARTER_HOURLY_CONSUMING.value
